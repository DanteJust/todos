...
const getItem = async (req: Request, res: Response) => {
    ...
    return Item.findOne({ _id: item_id, list_id: list_id })
...
const deleteItem = async (req: Request, res: Response) => {
    ...
    return Item.deleteOne({ _id: item_id, list_id: list_id })
...
const updateItem = async (req: Request, res: Response) => {
    ...
    return Item.updateOne(
        { _id: item_id, list_id: list_id },
        { $set: { text: text } }
    )
        .then(() =>
            res
                .status(200)
                .json({ message: "Item has been successfully updated!" })
        )
        .catch((error) => res.status(500).json({ error }));
};
...
const updateItemStatus = async (req: Request, res: Response) => {
    ...
    return Item.updateOne(
        ...
        .then(() =>
            res
                .status(200)
                .json({ message: "Item status has been successfully updated!" })
        )
        .catch((error) => res.status(500).json({ error }));
};
