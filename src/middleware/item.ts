...
const checkPayloadTypesAndPresence = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    ...
    if (status) {
        if (typeof status !== "string") {
            return res
                .status(500)
                .json({ message: "All fields must be string!" });
        }
    }
...
const checkPayloadTypesAndPresenceTextUpdate = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
...
const checkPayloadTypesAndPresenceStatusUpdate = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    ...
    if (typeof status !== "string") {
        return res
            .status(500)
            .json({ message: "Status field must be string!" });
    }
...
export default {
    verifyItemPath,
    checkPayloadTypesAndPresence,
    checkPayloadTypesAndPresenceTextUpdate,
    checkPayloadTypesAndPresenceStatusUpdate,
};